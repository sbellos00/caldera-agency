  ...
  return (
    <>
      ...
      <div ref={scrollContainerRef} className="relative z-10 theme-container" style={{ backgroundColor: 'rgb(250, 248, 245)', color: 'rgb(0, 0, 0)', transition: 'background-color 400ms ease-out' }}>
        ...
        <section ref={el => { themedSectionRefs.current[4] = el }} id="faq" className="relative overflow-hidden bg-[#0019ff] text-white">
              ...
              {(showAllFAQ ? faqData : faqData.slice(0, 5)).map((item, i) => (
                <div key={i} className={i < 5 ? 'scroll-fade' : ''}>
                  <button onClick={() => setOpenFAQ(openFAQ === i ? null : i)}
                    className="w-full bg-white/10 backdrop-blur-sm hover:bg-white border border-white/20 hover:border-white rounded-xl md:rounded-2xl p-4 md:p-6 text-left transition-all duration-300 hover:shadow-2xl group">
                    <div className="flex items-center justify-between">
                      <h3 className="text-base md:text-lg lg:text-xl font-normal pr-6 text-white group-hover:text-[#0019ff] transition-colors duration-300">{item.q}</h3>
                      <div className={`w-5 h-5 flex items-center justify-center transition-transform duration-300 ${openFAQ === i ? 'rotate-45' : ''}`}>
                        <span className="text-xl font-light leading-none text-white group-hover:text-[#0019ff] transition-colors duration-300">+</span>
                      </div>
                    </div>
                  </button>
